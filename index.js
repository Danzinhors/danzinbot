...
limitawal = 30
memberlimit = 0
ator = '🔥 ⃨⃰⃟⃤ᴅᴀɴᴢɪɴ🔥 ⃨⃰⃟⃤'
namo = '🔥 ⃨⃰⃟⃤ᴅᴀɴᴢɪɴ🔥 ⃨⃰⃟⃤'
cr = '*DANZIN-BOT*'
/*************************************/
...
client.on('group-participants-update', async (anu) => {
	...
	const mdata = await client.groupMetadata(anu.jid)
			if (anu.action == 'add'){
				...
				client.sendMessage(mdata.id, '👮🏻‍♀️Sem número fake aqui pilantra 👮🏻‍♀️️', MessageType.text)
      ...
      try {
        ...
        shortgc = await axios.get(`https://tinyurl.com/api-create.php?url=${ppgc}`)
         if (anu.action == 'add') {
            ...
            teks = `◪ Nem Pagou Nossa Coca😔🖐️ ${namaewa}\n◪ Saiu de:\n${mdata.subject}\n\n└─ ❏ Número: ${num.replace('@s.whatsapp.net', '')}\nJa vai tarde baka kkkk~~`
            client.sendMessage(mdata.id, img, MessageType.image, {caption: teks, contextInfo: {'mentionedJid': [num]}})
            ...
            teks = `◪ PROMOVER DETECTADO\n\n├─ Número: ${num.replace('@s.whatsapp.net', '')}\n├─ Mensagem: @${num.split('@')[0]} virou adm do grupo né mizera , parabéns`
            client.sendMessage(mdata.id, img, MessageType.image, {caption: teks, contextInfo: {'mentionedJid': [num]}})
            ...
            teks = `◪ DESPROMOVER DETECTADO\n\n\n├─ Número: ${num.replace('@s.whatsapp.net', '')}\n├─  @${num.split('@')[0]} Vacilou perdeu adm do grupo, seu otaro`
            client.sendMessage(mdata.id, img, MessageType.image, {caption: teks, contextInfo: {'mentionedJid': [num]}})
         }
     } catch (e) {
...
case 'spam':
    			if (!isRegistered) return reply(ind.noregis())
		    		if (isLimit(sender)) return reply(ind.limitend(pusname))
			     	await limitAdd(sender)	    				    					     	
                    if (args.length < 1) return reply(`Use assim:  exemplo : ${prefix}spam 558486067295 corno`)
...
pp_riu = await getBuffer(pp_user)
		         ...
		         reply(`[❗] Olá ${pushname} aguarde⏳`)                        
					num = `${sender.split("@")[0]}@s.whatsapp.net`
		    ow = "558486067295@s.whatsapp.net"
		    cuy = "558486067295@s.whatsapp.net"
                    menusu = `
❏ *MENU MAKER*
│
└◪› ${prefix}crosslogo
...
break
         	   ...
         	   case 'creator':
		    		...
		    		if (isLimit(sender)) return reply(ind.limitend(pusname))
			     	await limitAdd(sender)   	   
                  client.sendMessage(from, {displayname: "Dan", vcard: vcard}, MessageType.contact, { quoted: mek})
                  client.sendMessage(from, 'Esse é o número do meu criador 😎',MessageType.text, { quoted: mek} )
					break    
...
break
    ...
    case 'transferi':
                ...
                addKoinUser('558486067295@s.whatsapp.net', fee)
...
.on('end', function () {
console.log('Finish')
exec(`webpmux -set exif ${addMetadata('DANZIN-BOT', '🔥 ⃨⃰⃟⃤ᴅᴀɴᴢɪɴ🔥 ⃨⃰⃟⃤')} ${ran} -o ${ran}`, async (error) => {
...
exec(`webpmux -set exif ${addMetadata('DANZIN-BOT', '🔥 ⃨⃰⃟⃤ᴅᴀɴᴢɪɴ🔥 ⃨⃰⃟⃤')} ${ranw} -o ${ranw}`, async (error) => {
if (error) return reply(ind.stick())
...
.on('error', function (err) {
console.log(`Error : ${err}`)
exec(`webpmux -set exif ${addMetadata('DANZIN-BOT', '🔥 ⃨⃰⃟⃤ᴅᴀɴᴢɪɴ🔥 ⃨⃰⃟⃤')} ${rano} -o ${rano}`, async (error) => {
fs.unlinkSync(media)
reply(ptbr.stick())
})
})
		  ...
		  if (!isBotGroupAdmins) return reply(dpa.badmin)
				...
				case 'tm': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `❮ ᴅᴀɴᴢɪɴ ʙᴏᴛ ❯\n\n${body.slice(4)}`})
						}
						reply('[TM] de foto feita com sucesso ✔')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `❮ ᴅᴀɴᴢɪɴ ʙᴏᴛ ❯\n\n${body.slice(4)}`)
						}
